...
| 5 | ~~Online multiplayer~~ — door de opdrachtgever afgewezen (app blijft serverloos). In de plaats: **scorebord voor een fysiek kaartspel** — houd per ronde de punten bij. Twee modi: **manueel** (2–6 deelnemers, vrije punten) en **wiezen (auto)** (later uitgebreid naar alle spellen, zie fase 14) — duid contract (incl. troel), speler + maat en aantal slagen aan, de app berekent de punten via de scoring-engine. Persistent in localStorage | ✅ |
...
| 13 | **Frans tarot** — het apart traject: een **eigen kaartset van 78 kaarten** (`engine/tarot-cards.ts`: vier kleuren van veertien met cavalier, 21 atouts, de excuse) en een **variabel aantal spelers** (3, 4 of 5), waar elke andere engine op vier stoelen vastzit. [REGELS-TAROT.md](REGELS-TAROT.md), `rulesets/tarot.json` en een eigen engine die het **officiële FFT-reglement** volgt: delen per drie tegen de klok in met de chien tijdens het delen opgebouwd, **petit sec** (donne geannuleerd), vier contracten (petite ×1 t.e.m. garde contre ×6), de chien met de écart-beperkingen, bij vijf spelers de **geroepen heer**, troef- én stijgplicht, de excuse die nooit wint maar in je eigen slagen blijft (ook in de laatste slag bij een chelem), doelen van 56/51/41/36, **petit au bout** (×factor), **poignée** (20/30/40 met drempels per spelersaantal, naar de winnaar van de gift) en **chelem** (aangekondigd +400, stil +200, gemist −200). Alles in **halve punten** zodat er nooit afgerond hoeft te worden. Zes **regelvarianten** kies je op het startscherm (§11 in de regels). Bots, persistentie, coach, gidshoofdstuk en wizard in nl/en/fr | ✅ |
| 14 | **Scorebord: automodus voor elk spel** — het scorebord voor een fysiek kaartspel kende enkel wiezen. Nu heeft **elk** van de acht spellen een automodus: je duidt aan wat er gebeurde en de app rekent de punten uit. De scorelogica is daarvoor uit de engines getrokken (`scoreKlaverjasRound`, `scoreBeloteRound`, `scoreHartenRound`, `scoreManilleGift`, `scoreBiedenGift`, `scoreTarotRound`), zodat het bord aan tafel exact hetzelfde telt als het spel tegen de bots. Elk spel beschrijft zijn invoer als een lijstje velden (`scorebord-games.ts`); `main.ts` tekent die generiek, met een voorbeeld dat live meerekent terwijl je typt. De **moduskeuze volgt het aantal deelnemers**: bij drie of vijf zie je enkel boerenbridge en tarot, bij vier alles, bij twee enkel manueel | ✅ |
