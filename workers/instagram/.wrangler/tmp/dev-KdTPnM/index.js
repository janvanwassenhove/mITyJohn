...
async function logFailure(where, res) {
  let detail = "";
  try {
  ...
  console.error(`[ig] ${where} failed: HTTP ${res.status} ${detail}`);
}
...
async function refreshToken(env) {
  ...
  if (!res.ok) {
    await logFailure("refresh_access_token", res);
    return;
  }
...
// .wrangler/tmp/bundle-GsLAua/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-GsLAua/middleware-loader.entry.ts
